...
define('NICUPLOAD_URI', $_SERVER['PHP_SELF']);   // Set the URL (relative or absolute) to
...
//$sciezka = $_SERVER['SERVER_NAME'].$_SERVER['PHP_SELF'];
//$sciezka = $_SERVER['HTTP_HOST'];
function nicupload_file_uri($filename) {
 ...
 //if ($path_count == 2) $path = 'hej./';
 //else if ($path_count == 3) $path = '../';
 //echo '<br>';
 $katalog = $path_tab[1];
    return 'http://'.$_SERVER['SERVER_NAME'].'/'.$katalog.'/images/'.$filename;
    //return NICUPLOAD_PATH.'/'.$filename;
}
